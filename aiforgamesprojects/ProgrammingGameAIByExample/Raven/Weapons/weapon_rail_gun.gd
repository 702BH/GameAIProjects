...
func shoot_at(pos: Vector2) -> void:
	if num_rounds_left > 0 and is_ready_for_next_shot():
		var bullet = ProjectileSlug.new(pos, owner_agent)
...
func initialise_fuzzy_module() -> void:
	...
	var ammo_loads : FzSet = ammo_status.add_right_shoulder_set("Ammo_Loads", 10, 30, 100)
	var ammo_okay : FzSet = ammo_status.add_triangular_set("Ammo_Okay", 0, 10, 30)
	var ammo_low : FzSet = ammo_status.add_triangular_set("Ammo_Low", 0, 0, 10)
	
	fuzzy_module.add_rule(FuzzyAnd.new([target_close, ammo_loads]), FzFairly.new(desirable))
	fuzzy_module.add_rule(FuzzyAnd.new([target_close, ammo_okay]),  FzFairly.new(desirable))
	...
	fuzzy_module.add_rule(FuzzyAnd.new([target_far, ammo_loads]), FzVery.new(very_desirable))
	fuzzy_module.add_rule(FuzzyAnd.new([target_far, ammo_okay]), FzVery.new(very_desirable))
	fuzzy_module.add_rule(FuzzyAnd.new([target_far, FzFairly.new(ammo_low)]), very_desirable)
